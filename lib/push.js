...
async function sendTo(subs, payload, model = 'pushSubscription') {
  ...
  if (dead.length) await prisma[model].deleteMany({ where: { id: { in: dead } } }).catch(() => {});
...
// Alerts for the admin app (every admin device that turned them on).
async function pushToAdmins(title, message, url = '/admin') {
  try {
    const subs = await prisma.adminPushSubscription.findMany();
    return await sendTo(subs, { title, body: message, url }, 'adminPushSubscription');
  } catch (error) {
    console.error('pushToAdmins failed:', error.message);
    return { sent: 0 };
  }
}

module.exports = { publicKey, pushToCustomer, pushToAll, pushToAdmins };
