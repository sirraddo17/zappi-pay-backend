...
async function finalize(transfer, providerStatus, reason) {
  ...
  if (!status) {
    ...
    if (nextStatus === 'PENDING_AUTHORIZATION') {
      // First time it needs an OTP: tell the admins straight away.
      const r = await prisma.bankTransfer.updateMany({ where: { id: transfer.id, status: 'PROCESSING' }, data: { status: nextStatus, providerStatus } });
      if (r.count === 1) {
        alertAdmins('Bank transfer needs your OTP', `${money(transfer.amount)} to ${transfer.accountName} (${transfer.bankName || transfer.bankCode}). Enter the Monnify OTP to release it.`, '/admin/bank-transfers');
...
const lastTransferCheck = new Map();
async function refreshCustomerPending(customerId) {
  return refreshPendingTransfers({ customerId });
}

// Re-checks PROCESSING transfers with Monnify (customer's own list, or
// all of them for the admin page).
async function refreshPendingTransfers(where = {}) {
  const pending = await prisma.bankTransfer.findMany({
    where: { ...where, status: 'PROCESSING', createdAt: { lt: new Date(Date.now() - 30 * 1000) } },
    orderBy: { createdAt: 'desc' },
    take: 5,
  });
  for (const t of pending) {
    // The app re-checks every few seconds while a transfer is
    // processing; ask Monnify about each one at most every 15 seconds.
    const now = Date.now();
    if (now - (lastTransferCheck.get(t.id) || 0) < 15 * 1000) continue;
    lastTransferCheck.set(t.id, now);
    if (lastTransferCheck.size > 5000) lastTransferCheck.clear();
    try {
...
async function sendToBank(customerId, { bankCode, accountNumber, amount, narration }) {
  ...
  if (reasons.length) {
    ...
    alertAdmins(
      `Bank transfer held for review (${money(amt)})`,
      `${money(amt)} to ${acct.accountName} (${bankName || bankCode} ${acct.accountNumber}).\nReason: ${reasons.join('; ')}\nRef ${reference}\nRelease or refund it under Bank Transfers → Held.`,
