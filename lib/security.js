...
async function registrationOptions(req, customer, device, { compat = false } = {}) {
  ...
  const options = await generateRegistrationOptions({
    ...
    // A device-bound (non-discoverable) credential is all we need —
    // login always names the credential via allowCredentials. Asking
    // for a discoverable passkey makes Android route the request to
    // Google Password Manager, which fails with "unknown error ...
    // credential manager" on many phones. ES256/RS256 only, since some
    // Android authenticators reject the default Ed25519 entry.
    // `compat` (the automatic retry) also drops the platform-only hint.
    supportedAlgorithmIDs: [-7, -257],
    authenticatorSelection: {
      ...(compat ? {} : { authenticatorAttachment: 'platform' }),
